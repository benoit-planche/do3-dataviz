---
title: "TP 1.1 - Introduction à `R markdown`"
author: "Paul Bastide"
date: "02/04/2024"
...
`R Markdown` permet de créer des documents dynamiques, qui incluent du
texte mis en forme, des équations, et du code `R`.
...
à `R markdown`, ainsi que la [fiche
synthétique](https://raw.githubusercontent.com/rstudio/cheatsheets/master/rmarkdown-2.0.pdf).
...
nouveau document en utilisant l'interface de `RStudio`.
...
Dans l'onglet `Document` (sélectionné par défaut), vous pouvez saisir le
titre et la, le ou les auteur·e·s du document.

Trois formats sont proposés:

-   `HTML`: permet une mise en forme dynamique, utile pour les sites
    internet.

-   `PDF`: permet une mise en forme fixe, utile pour un rendu "officiel"
    ou papier.

-   `Word`: permet de générer des documents éditables, qui peuvent
    ensuite être partagés avec des collaborateurs ou collaboratrices.

Pour cette séance, on choisi le format `HTML`, le plus simple pour la
mise en page.

## Premier document

Une fois le document créé, `RStudio` propose un contenu "didactique" par
défaut.

Vous pouvez compiler ce document en cliquant sur la commande `Knit` en
haut à gauche, à côté d'une pelote de laine (en anglais, "to knit"
signifie "tricoter").

Si tout fonctionne bien, `RStudio` va générer un document `HTML`, qu'il
ouvre dans une nouvelle fenêtre.

Vous pouvez étudier ce premier document, et voir comment la source (le
`.Rmd`) influe sur la sortie (le `.html`).
...
Avec `R`, on peut simuler cette expérience aléatoire de la façon
suivante.
...
Par exemple, sur les `r n_exp` expériences, on a tiré ici `r res[1]`
fois la boule bleue.
...
Rédigez un document `R markdown` qui réponde au problème exposé
...
Il pourra contenir des équations $\LaTeX$, et des listes, numérotées ou
non.
...
> pollué). On suppose cependant que l'on n'a pas accès à cette
> information (on ne sait pas combien il y a de poissons en tout, ni
> combien de chaque couleur).
>
...
> a-t-elle bien des modalités incompatibles, exhaustives et sans
> ambiguïté ?
>
...
> utilisant la fonction `table`. Quelle est la proportion estimée de
> poissons rouges, verts et bleus ? Ce résultat vous surprend-il ?
>
> Tracer ces fréquences empiriques en utilisant la fonction `barplot`.
>
> Que se passe-t-il lorsque vous compilez votre document plusieurs fois
> de suite ? Les résultats numériques changent-ils ? Est-ce normal ?
> Utilisez la fonction `set.seed` pour produire un document
> reproductible.
>
